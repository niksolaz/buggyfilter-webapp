  ...
  <div>
    ...
    <form class="flex flex-col gap-4" @submit.prevent="submit">
      <UFormField label="Email" name="email" required>
        <UInput v-model="form.email" type="email" placeholder="nome@azienda.com" icon="i-lucide-mail"
          autocomplete="email" required class="w-full" />
      </UFormField>
      ...
      <UButton type="submit" label="Accedi" block :loading="loading" />
    </form>

    <USeparator label="oppure accesso rapido" class="my-4" />

    <div class="flex gap-2">
      <UButton label="Operatore" icon="i-lucide-wrench" variant="outline" color="neutral" block :disabled="loading"
        @click="quickLogin('operator@buggyfilter.dev')" />
      <UButton label="Cliente" icon="i-lucide-building-2" variant="outline" color="neutral" block :disabled="loading"
